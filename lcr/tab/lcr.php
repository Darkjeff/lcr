...
/**
 *	\file       htdocs/compta/facture/lcr.php
 *	\ingroup    facture
 *	\brief      Gestion des lcr d'une facture
 */
...
if ($action == "new"/* || $action == "newescompte"*/ )
{
    if ($object->id > 0)
    {
        $result = $object->demande_lcr($user, GETPOST('bankdraft_request_amount'));
...
if ($action == "delete")
{
    if ($object->id > 0)
    {
        $result = $object->demande_lcr_delete($user, GETPOST('did'));
        if ($result == 0) {
            header("Location: ".$_SERVER['PHP_SELF']."?id=".$object->id);
            exit;
        }
...
llxHeader('', $langs->trans("BankdraftBills"));
...
if ($object->id > 0)
{
	...
	dol_fiche_head($head, 'lcrtab', $langs->trans('InvoiceCustomer'),0,'bill');
	...
	$sql = "SELECT pfd.rowid, pfd.traite, pfd.date_demande as date_demande,pfd.date_lim_reglement";
	...
	if ( $resteapayer >= 0 )
	{
		if ($user->rights->lcr->bons->creer)
		{
			print '<form method="POST" action="">';
			print '<input type="hidden" name="id" value="' . $object->id . '" />';
			print '<input type="hidden" name="action" value="new" />';

			print '<td>Date échéance  </td>';
       		print $form->select_date($object->date_lim_reglement, '', '', '', '', "add", 1, 1, 1); 

			print '<label for="bankdraft_request_amount">' . $langs->trans('BankdraftRequestAmount') . ' </label>';
			print '<input type="text" id="bankdraft_request_amount" name="bankdraft_request_amount" value="' . $resteapayer . '" size="10" />';
			print '<input type="submit" class="butAction" value="'.$langs->trans("MakeBankdraftRequest").'" />';
			print '</form>';
		}
		else
		{
			print '<a class="butActionRefused" href="#">'.$langs->trans("MakeBankdraftRequest").'</a>';
		}
	}
	else
	{
	...
	print $langs->trans("DoStandingOrdersBeforePayments")." LCR".'<br><br>';
	...
	print '<td align="center">'.$langs->trans("BankdraftAmount").'</td>';
	print '<td align="center">'.$langs->trans("BankdraftReceipts").'</td>';
	...
	if ($result_sql)
	{
		...
		while ($i < $num)
		{
			...
			print '<td align="center">'.$langs->trans("BankdraftOrderWaiting").'</td>';
	...
	$sql = "SELECT pfd.rowid, pfd.traite, pfd.date_demande,pfd.date_lim_reglement,";
	$sql.= " pfd.date_traite, pfd.fk_lcr_bons, pfd.amount,";
	$sql.= " pb.ref,";
	$sql.= " u.rowid as user_id, u.lastname, u.firstname, u.login";
	$sql.= " FROM ".MAIN_DB_PREFIX."lcr_facture_demande as pfd,";
	...
	$sql.= " WHERE fk_facture = ".$object->id;
	$sql.= " AND pfd.fk_user_demande = u.rowid";
	$sql.= " AND pb.rowid = pfd.fk_lcr_bons";
	...
	if ($result)
	{
		...
		while ($i < $num)
		{
			...
			$Bankdraftreceipt=new BonLcr($db);
			$Bankdraftreceipt->id=$obj->fk_lcr_bons;
			$Bankdraftreceipt->ref=$obj->ref;
			print $Bankdraftreceipt->getNomUrl(1);
			print "</td>\n";
