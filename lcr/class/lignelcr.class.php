...
class LigneLcr
{
	...
	function __construct($db, $user)
	{
		...
		$langs->load("lcr");
		$this->statuts[0]=$langs->trans("BankdraftStatusWaiting");
		$this->statuts[2]=$langs->trans("BankdraftStatusCredited");
		$this->statuts[3]=$langs->trans("BankdraftStatusRefused");
	}
	 ...
	 *  Take object lcr
	 *
	 *  @param	int		$rowid       id of invoice to take
	 *  @return	void
	 */
	...
	function fetch($rowid)
	{
		...
		$sql.= ", ".MAIN_DB_PREFIX."lcr_bons as p";
		$sql.= " WHERE pl.rowid=".$rowid;
		$sql.= " AND p.rowid = pl.fk_lcr_bons";
		...
		if ($resql)
		{
			...
			{
				$result++;
				dol_syslog("LigneLcr::Fetch rowid=$rowid numrows=0");
			}
		...
		{
			$result++;
			dol_syslog("LigneLcr::Fetch rowid=$rowid");
